...
for (i in cluster.name){
  ...
  ggsave(dimp, file = paste0(path,"/output/6_Bcell_clusters/umap_comparison/DimPlot_Bcell_", i, ".png"),
         height = 3, width = 3.5, units = "in", bg = 'white')
}

features <- rownames(Bcell@assays$ADT)
for(i in features){
  fp=FeaturePlot(Bcell, features =i, cols =  c(alpha("lightgrey",0.4), "blue"), pt.size =1, min.cutoff =1 ,max.cutoff =5)
  ggsave(fp, file = paste0(path, "/output/6_Bcell_clusters/FeaturePlot_1/FeaturePlot_", i, ".png"), 
...
ggsave(file = paste0(path,"/output/6_Bcell_clusters/DimPlot_Bcell_monaco.png"),
...
cluster_name = c("1:naive","2:memory_CD11c+","3:memory")
...
# *Save RDS file* 
saveRDS(Bcell, file = paste0(path, "/output/6_Bcell_clusters/Bcell_clusters.rds"))
...
ggsave(file = paste0(path,"/output/6_Bcell_clusters/DimPlot_Bcell_cluster_name0.3.png"),
...
ggsave(file = paste0(path,"/output/6_Bcell_clusters/DimPlot_Bcell_SampleType.png"),
...
for(i in features){
  FeaturePlot(Bcell, features =i, cols =  c(alpha("lightgrey",0.4), "blue"),label = T,  
    ...
    NoAxes()
  ggsave(file = paste0(path, "/output/6_Bcell_clusters/FeaturePlot_2/FeaturePlot_", i, ".png"), 
...
write.csv(df_count, file = paste0(path,"/output/6_Bcell_clusters/Bcell_cluster_counts_patient.csv"), row.names = F) 
...
df_prop <- df_prop %>% pivot_wider(names_from = 'cluster_name0.3', values_from = 'num')
...
write.csv(df_prop, file = paste0(path,"/output/6_Bcell_clusters/Bcell_cluster_proportion_patient.csv"), row.names = F) 


## DGE: MAST
Idents(Bcell) <- "cluster_0.3"
one_vs_all_rna <- FindAllMarkers(Bcell, assay = "RNA", test.use = "MAST", only.pos = F, latent.vars = "SampleName", min.pct = 0.1)
write.csv(one_vs_all_rna, file = paste0(path,"/output/6_Bcell_clusters/dge/Bcell_one_vs_all_rna_MAST.csv"))
...
write.csv(one_vs_all_rna, file = paste0(path,"/output/6_Bcell_clusters/dge/Bcell_one_vs_all_rna_cluster_name0.3_MAST.csv"))
...
ggplot(df.dot, aes(x = id, y = features.plot)) +
  ...
  xlab("") +ylab("")
ggsave(file = paste0(path,"/output/6_Bcell_clusters/DotPlot_Bcell_cluster_ADT_markers.png"),
       height = 5, width = 3.3, units = 'in', bg = 'white')

## Heatmap for DEGs
dge <- read.csv(file = paste0(path,"/output/6_Bcell_clusters/dge/Bcell_one_vs_all_rna_MAST.csv"), row.names = 1)
...
col <-scales::pal_brewer(palette ="Set1")(3)
cluster <-col
...
# *Figure S5C*
png(filename = paste0(path, "/output/6_Bcell_clusters/Heatmap_Bcell_cluster_name0.3_MAST.png"), height =3.5, width =8.5, res=500, units = "in") 
pheatmap::pheatmap(hm.mat, 
                   color = pal_cols(100), 
                   fontsize = 13, fontsize_row = 13, fontsize_number=13, 
...
Bcell$SampleType <- factor(Bcell$SampleType, levels = c("Normal_HTx","CAV"))
set.seed(47)
sccomp_result <- Bcell %>% 
...
plots$credible_intervals_1D[[1]] +
  ...
  ggtitle("") 
ggsave(file = paste0(path,"/output/6_Bcell_clusters/Bcell_cell_composition_sccomp.png"), 
