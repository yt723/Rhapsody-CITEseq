...
#load the data
CD8T <- readRDS(file = paste0(path, "/output/9_CD38_HLADR/CD8T_diffusion_map_CD38HLADR.rds"))
vdj_df <- read.csv(file = paste0(path,"/raw_data/vdj_combined.csv"), row.names = 1)
...
# save RDS file 
saveRDS(CD8T, file = paste0(path, "/output/10_clonotype/CD8T_clonotype.rds"))
...
ggsave(file = paste0(path, "/output/10_clonotype/R_E_curve_q1_CD8T_type3.png"),
...
ggsave(file = paste0(path, "/output/10_clonotype/CD8T_diversity_SampleType.png"),
...
ggsave(file = paste0(path, "/output/10_clonotype/DPT_CD8T_Shannon_CD38HLADR.png"),
...
FeaturePlot(CD8T, features = 'log_cell_count', reduction = 'wnn.umap', order = T,
  ...
  labs(color = "Size of\nclonotypes\nlog10 value") +
  ggtitle("") 
ggsave(file = paste0(path, "/output/10_clonotype/FeaturePlot_CD8T_clone_log10.png"),
       width = 7, height=5, units = "in", bg = 'white')
...
ggsave(file = paste0(path, "/output/10_clonotype/CD8T_clone_log10count_cluster.png"),
...
ggsave(file = paste0(path, "/output/10_clonotype/CD8T_clone_overlap_log2FC_cluster.png"),
...
ggplot(df_out, aes(Var2, Var1, , fill=value))+
  ...
  xlab("") + ylab("")+
  coord_fixed()
ggsave(file = paste0(path, "/output/10_clonotype/CD8T_heatmap_jaccard_clusters.png"),
...
ggsave(file = paste0(path, "/output/10_clonotype/CD8T_jaccard_C4_vs_other.png"),
       width = 5.5, height=4.5, units = "in", bg = 'white')
...
# save RDS file 
saveRDS(CD4T, file = paste0(path, "/output/10_clonotype/CD4T_clonotype.rds"))
...
ggsave(file = paste0(path, "/output/10_clonotype/CD4T_diversity_SampleType.png"),
...
# save RDS file 
saveRDS(Bcell, file = paste0(path, "/output/10_clonotype/Bcell_clonotype.rds"))
...
ggsave(file = paste0(path, "/output/10_clonotype/Bcell_diversity_SampleType.png"),
       width = 4.2, height=2.5, units = "in", bg = 'white')
