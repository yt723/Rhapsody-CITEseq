...
# subset NK cells
NK <- subset(pbmc, major_celltype %in% c("Natural killer"))
...
for (i in cluster.name){
  ...
  ggsave(file = paste0("output/7_NK_clusters/umap_comparison/DimPlot_NK_", i, ".png"),
...
for(i in features){
  ...
  ggsave(file = paste0("output/7_NK_clusters/FeaturePlot_1/FeaturePlot_", i, ".png"), 
...
# *Save RDS file* 
saveRDS(NK, file = "output/7_NK_clusters/NK_clusters.rds")
...
ggsave(file = "output/7_NK_clusters/DimPlot_NK_cluster_name0.3.png",
...
DimPlot(NK, group.by ="SampleType", label = F, pt.size =0.05) + 
  ...
  NoAxes() + ggtitle("")
ggsave(file = "output/7_NK_clusters/DimPlot_NK_SampleType.png",
...
for(i in features){
  FeaturePlot(NK, features =i, cols =  c(alpha("lightgrey",0.4), "blue"),label = T,  
    ...
    NoAxes()
  ggsave(file = paste0("output/7_NK_clusters/FeaturePlot_2/FeaturePlot_", i, ".png"), 
...
write.csv(df_count, file = "output/7_NK_clusters/NK_cluster_counts_patient.csv", row.names = F) 
...
write.csv(df_prop, file = "output/7_NK_clusters/NK_cluster_proportion_patient.csv", row.names = F) 

## Differential composition analysis (*Figure 2D*)
library(sccomp)

NK$SampleType <- factor(NK$SampleType, levels = c("Normal HTx","CAV"))
set.seed(47)
sccomp_result <- NK %>% 
...
plots$credible_intervals_1D[[1]] +
  ...
  ggtitle("") 
ggsave(file = "output/7_NK_clusters/NK_cell_composition_sccomp.png", 
...
write.csv(one_vs_all_rna, file = "output/7_NK_clusters/dge/NK_one_vs_all_rna_MAST.csv")
...
write.csv(one_vs_all_rna, file = "output/7_NK_clusters/dge/NK_one_vs_all_rna_cluster_name0.3_MAST.csv")
...
ggplot(df.dot, aes(y = id, x = features.plot)) +
  ...
  xlab("") +ylab("")
ggsave(file = "output/7_NK_clusters/DotPlot_NK_cluster_ADT_markers.png",
       height = 3, width = 5, units = 'in', bg = 'white')

## Heatmap for DGE
dge <- read.csv(file = "output/7_NK_clusters/dge/NK_one_vs_all_rna_MAST.csv", row.names = 1)
...
# *Figure S7C* 
png(filename = "output/7_NK_clusters/Heatmap_NK_cluster_name0.3.png", height =3.5, width =10, res=500, units = "in") 
