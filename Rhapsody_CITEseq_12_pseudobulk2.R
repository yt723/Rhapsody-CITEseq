...
CD4T <- readRDS(file = paste0(path, "/output/5_CD4T_clusters/CD4T_clusters.rds"))
NK <- readRDS(file = paste0(path, "/output/7_NK_clusters/NK_clusters.rds"))
...
ggplot(dat.summary, aes(y=marker, x=cluster)) +
  ...
  xlab("") + ylab("")
ggsave(file = paste0(path, "/output/12_pseudobulk2/dotplot_cytotoxic_CD8_CD4_NK_clusters.png"),
       width = 5, height = 3, units = "in", bg = 'white')

## SampleType comparison 
# CD8T 
cts <- AggregateExpression(CD8T, assays = "RNA", group.by = c("SampleName"), return.seurat = F)
cts <- cts$RNA

col.order <- colnames(cts)
colData <- data.frame(sample = colnames(cts))
rownames(colData) <- colData$sample
colData$SampleName <- colData$sample
meta <- distinct(CD8T@meta.data[,c("SampleName", "Library", "SampleType")])
meta$SampleName <- gsub("_","-", meta$SampleName)
...
obj <- DGEList(counts = cts, samples = colData)
head(obj$samples, 25)
keep <- filterByExpr(obj, group = colData$SampleName)
summary(keep)
obj <- obj[keep,]
obj <- calcNormFactors(obj)
plotMDS(obj)

mm <- model.matrix(~ 0+ factor(SampleType), obj$samples)
colnames(mm) <- c("CAV", "Normal_HTx")

obj <- estimateDisp(obj, mm)
plotBCV(obj)
fit <- glmQLFit(obj, mm, robust = T)

contr <- makeContrasts("CAV-Normal_HTx", levels = mm)
lrt <- glmQLFTest(fit, contrast = contr)

de.results <- lrt$table

pval <- de.results$PValue
names(pval) <- rownames(de.results)
pval <- p.adjust(pval, method = 'fdr')
de.results$fdr <- pval
de.results <- de.results[de.results$PValue <0.05,]

# *Excel file 5*
write.csv(de.results, file = paste0(path, "/output/12_pseudobulk2/edgeR_dge_SampleType_CD8T.csv"))


# CD4T 
cts <- AggregateExpression(CD4T, assays = "RNA", group.by = c("SampleName"), return.seurat = F)
cts <- cts$RNA
...
rownames(colData) <- colData$sample
colData$SampleName <- colData$sample
meta <- distinct(CD4T@meta.data[,c("SampleName", "Library", "SampleType")])
meta$SampleName <- gsub("_","-", meta$SampleName)
...
# *Excel file 5*
write.csv(de.results, file = paste0(path, "/output/12_pseudobulk2/edgeR_dge_SampleType_CD4T.csv"))

# Bcell
Bcell <- readRDS(file = paste0(path, "/output/6_Bcell_clusters/Bcell_clusters.rds"))
cts <- AggregateExpression(Bcell, assays = "RNA", group.by = c("SampleName"), return.seurat = F)
cts <- cts$RNA
...
rownames(colData) <- colData$sample
colData$SampleName <- colData$sample
meta <- distinct(Bcell@meta.data[,c("SampleName", "Library", "SampleType")])
meta$SampleName <- gsub("_","-", meta$SampleName)
...
# *Excel file 5*
write.csv(de.results, file = paste0(path, "/output/12_pseudobulk2/edgeR_dge_SampleType_Bcell.csv"))

# NK
cts <- AggregateExpression(NK, assays = "RNA", group.by = c("SampleName"), return.seurat = F)
cts <- cts$RNA
...
rownames(colData) <- colData$sample
colData$SampleName <- colData$sample
meta <- distinct(NK@meta.data[,c("SampleName", "Library", "SampleType")])
meta$SampleName <- gsub("_","-", meta$SampleName)
...
# *Excel file 5*
write.csv(de.results, file = paste0(path, "/output/12_pseudobulk2/edgeR_dge_SampleType_NK.csv"))
