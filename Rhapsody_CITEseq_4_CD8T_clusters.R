...
ggsave(file = paste0(path, "/output/4_CD8T_clusters/Dimplot_CD8T_clusters_initial.png"), 
...
for(i in markers){
  ...
  ggsave(file = paste0(path, "/output/4_CD8T_clusters/FeaturePlot_1/FeaturePlot_", i, ".png"), 
...
# save RDS file 
saveRDS(CD8T, file = paste0(path, "/output/4_CD8T_clusters/CD8T_cleaned.rds"))
...
for (i in cluster.name){
  ...
  ggsave(file = paste0(path,"/output/4_CD8T_clusters/umap_comparison/DimPlot_CD8T_", i, ".png"),
...
ggsave(file = paste0(path,"/output/4_CD8T_clusters/DimPlot_CD8T_monaco.png"),
...
# Save RDS file 
saveRDS(CD8T, file = paste0(path, "/output/4_CD8T_clusters/CD8T_clusters.rds"))
...
ggsave(file = paste0(path,"/output/4_CD8T_clusters/DimPlot_CD8T_cluster_name0.3.png"),
...
ggsave(file = paste0(path,"/output/4_CD8T_clusters/DimPlot_SampleType.png"),
...
for(i in features){
  FeaturePlot(CD8T, features =i, cols =  c(alpha("lightgrey",0.4), "blue"),label = T,  
    ...
    NoAxes()
  ggsave(file = paste0(path, "/output/4_CD8T_clusters/FeaturePlot_2/FeaturePlot_", i, ".png"), 
...
write.csv(df_count, file = paste0(path,"/output/4_CD8T_clusters/CD8T_cluster_counts_patient.csv"), row.names = F) 
...
write.csv(df_prop, file = paste0(path,"/output/4_CD8T_clusters/CD8T_cluster_proportion_patient.csv"), row.names = F)
...
write.csv(one_vs_all_rna, file = paste0(path,"/output/4_CD8T_clusters/dge/CD8T_one_vs_all_rna_MAST.csv"))
...
write.csv(one_vs_all_rna, file = paste0(path,"/output/4_CD8T_clusters/dge/CD8T_one_vs_all_rna_cluster_name0.3_MAST.csv"))
...
ggplot(df.dot, aes(y = id, x = features.plot)) +
  ...
  theme(axis.text.x = element_text(size = 14, face='bold', angle = 270, hjust = 0, vjust=0.5),
        ...
        legend.key.spacing = unit(0.05, "in"))+
  xlab("") +ylab("")
ggsave(file = paste0(path,"/output/4_CD8T_clusters/DotPlot_CD8T_cluster_ADT_markers.png"),
       height = 3.5, width = 6, units = 'in', bg = 'white')

## Heatmap for DEGs
dge <- read.csv(file = paste0(path,"/output/4_CD8T_clusters/dge/CD8T_one_vs_all_rna_MAST.csv"), row.names = 1)
...
# *Figure 3C* 
png(filename = paste0(path, "/output/4_CD8T_clusters/Heatmap_CD8T_cluster_name0.3_MAST.png"), height =15, width =7.5, res=500, units = "in") 
...
ggplot(dat.summary, aes(x=marker, y=cluster_name)) +
  ...
  xlab("") +ylab("")
ggsave(file = paste0(path, "/output/4_CD8T_clusters/dotplot_cytotoxic_CD8T_cluster_pseudobulk.png"),
...
for (id in as.character(1:7)){
  ...
  write.csv(GO, file = paste0(path,"/output/4_CD8T_clusters/GO/CD8T_enrichGO_C",id, ".csv"))
}

go <- paste0("GO_C", as.character(1:7))
for (x in go){
  assign(x, read.csv(file = paste0(path,"/output/4_CD8T_clusters/GO/CD8T_enrich",x, ".csv")))
}
...
ggplot(GO.sub, aes(x = cluster,  y =str_wrap(Description, width = 30), colour  = zScore, size = log10.p_adj)) +
  ...
  theme(axis.text.x = element_text(size = 9, face='bold'),
        ...
        legend.key.spacing = unit(0.05, "in"))+
  xlab("") +ylab("")
ggsave(file = paste0(path,"/output/4_CD8T_clusters/DotPlot_CD8T_cluster_GO.png"),
       height = 3, width = 5.5, units = 'in', bg = 'white')
...
for (id in as.character(1:7)){
  ...
  ggplot(res, aes(x=avg_log2FC, y=-log10(p_val_adj), col=diff, label=gene))+
    ...
    ggtitle("")
  ggsave(file = paste0(path,"/output/4_CD8T_clusters/volcano/CD8T_volcano_C",id,"_pos_MAST.png"),
         width=3.5, height=3, units="in",bg = 'white')
  }
...
plots$credible_intervals_1D[[1]] +
  ...
  ggtitle("") 
ggsave(file = paste0(path,"/output/4_CD8T_clusters/CD8T_cell_composition_sccomp.png"), 
...
plots$credible_intervals_1D[[1]] +
  ...
  guides(color = guide_legend(title  = "FDR < significance threshold (0.025)")) +
  ggtitle("") 
ggsave(file = paste0(path,"/output/4_CD8T_clusters/CD8T_cell_composition_sccomp_Post_HTx_5y.png"), 
       width=3.5, height=2, units="in",bg = 'white')
