...
# subset CD4T cells
CD4T <- subset(pbmc, major_celltype %in% c("Naive_CD4T", "Memory_CD4T"))
...
for (i in cluster.name){
  ...
  ggsave(dimp, file = paste0(path,"/output/5_CD4T_clusters/umap_comparison/DimPlot_CD8T_", i, ".png"),
...
# *Save RDS file* 
saveRDS(CD4T, file = paste0(path, "/output/5_CD4T_clusters/CD4T_clusters.rds"))
...
ggsave(file = paste0(path,"/output/5_CD4T_clusters/DimPlot_CD4T_cluster_name0.3.png"),
...
ggsave(file = paste0(path,"/output/5_CD4T_clusters/DimPlot_CD4T_SampleType.png"),
       height = 2, width = 2, units = "in", bg = 'white')

# FeaturePlot (*Figure S4B*)
...
for(i in features){
  FeaturePlot(CD4T, features =i, cols =  c(alpha("lightgrey",0.4), "blue"),label = T,  
    ...
    NoAxes()
  ggsave(file = paste0(path, "/output/5_CD4T_clusters/FeaturePlot_1/FeaturePlot_", i, ".png"), 
...
write.csv(df_count, file = paste0(path,"/output/5_CD4T_clusters/CD4T_cluster_counts_patient.csv"), row.names = F) 
...
write.csv(df_prop, file = paste0(path,"/output/5_CD4T_clusters/CD4T_cluster_proportion_patient.csv"), row.names = F) 

## DGE: MAST
Idents(CD4T) <- "cluster_0.3"
one_vs_all_rna <- FindAllMarkers(CD4T, assay = "RNA", test.use = "MAST", only.pos = F, latent.vars = "SampleName", min.pct = 0.1)
write.csv(one_vs_all_rna, file = paste0(path,"/output/5_CD4T_clusters/dge/CD4T_one_vs_all_rna_MAST.csv"))
...
write.csv(one_vs_all_rna, file = paste0(path,"/output/5_CD4T_clusters/dge/CD4T_one_vs_all_rna_cluster_name0.3_MAST.csv"))
...
ggplot(df.dot, aes(x = id, y = features.plot)) +
  ...
  xlab("") +ylab("")
ggsave(file = paste0(path,"/output/5_CD4T_clusters/DotPlot_CD4T_cluster_ADT_markers.png"),
       height = 5.5, width = 3.3, units = 'in', bg = 'white')

## Heatmap for DEGs
dge <- read.csv(file = paste0(path,"/output/5_CD4T_clusters/dge/CD4T_one_vs_all_rna_MAST.csv"), row.names = 1)

dge$p_val_adj[which(dge$p_val_adj == 0)] <- 1e-300
dge$rank <- dge$avg_log2FC * (-1) * log10(dge$p_val_adj)

markers <- dge %>%  group_by(cluster) %>% top_n(7, rank)  %>% dplyr::arrange(desc(rank), .by_group = T)
...
col <- ggsci::pal_d3("category20")(20)[8:16]
cluster <-col
...
# *Figure S4C*
png(filename = paste0(path, "/output/5_CD4T_clusters/Heatmap_CD4T_cluster_name0.3_MAST.png"), height =4, width =14, res=500, units = "in") 
...
CD4T$SampleType <- factor(CD4T$SampleType, levels = c("Normal_HTx","CAV"))
set.seed(47)
sccomp_result <- CD4T %>% 
...
plots$credible_intervals_1D[[1]] +
  ...
  ggtitle("") 
ggsave(file = paste0(path,"/output/5_CD4T_clusters/CD4T_cell_composition_sccomp.png"), 
