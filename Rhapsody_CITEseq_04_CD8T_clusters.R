...
# subset CD8T cells
CD8T <- subset(pbmc, major_celltype %in% c("Naive CD8T", "Memory CD8T"))
...
DimPlot(CD8T, group.by ="SampleType", label = F, pt.size =0.05) + 
  ...
  NoAxes() + ggtitle("")
ggsave(file = "output/4_CD8T_clusters/DimPlot_CD8T_SampleType.png",
...
control <- df_prop[df_prop$SampleType == "Normal HTx",]
...
CD8T$SampleType <- factor(CD8T$SampleType, levels = c("Normal HTx","CAV"))
set.seed(47)
sccomp_result <- CD8T %>% 
...
# *Figure 3B* 
png(filename = "output/4_CD8T_clusters/Heatmap_CD8T_cluster_name0.3.png", height =4, width =16, res=500, units = "in") 
pheatmap::pheatmap(hm.mat, 
                   color = pal_cols(100), 
                   fontsize = 16, fontsize_row = 16, fontsize_number=16, 
                   labels_row = as.expression(newnames_row),
                   labels_col = as.expression(newnames_cols),
                   border_color = "white") 
dev.off()

## Volcano plot (*Figure 3C, S8A*)
library(ggrepel)
...
for (id in as.character(1:7)){
  ...
  res$diff <- ""
  res$diff[res$avg_log2FC>0.1 & res$p_val_adj<0.05] <- "up"
  res$diff[res$avg_log2FC<(-0.1) & res$p_val_adj<0.05] <- "down"
  mycolor <- c("red","blue")
  names(mycolor) <- c("up","down")
  
  for (x in 1:nrow(res)){
    if (abs(res$avg_log2FC[x]) >0.1 & res$p_val_adj[x]<0.05){
  ...
  ggplot(res, aes(x=avg_log2FC, y=-log10(p_val_adj), col=diff, label=gene))+
    ...
    ggtitle("")
  ggsave(file = paste0("output/4_CD8T_clusters/volcano/CD8T_volcano_C",id,"_MAST.png"),
         width=4, height=3.5, units="in", bg="white")
}
...
for (id in as.character(1:7)){
  ...
  GO.test <- enrichGO(gene = genes.test, OrgDb = "org.Hs.eg.db", ont = "BP", keyType = "SYMBOL", pAdjustMethod = "BH")
