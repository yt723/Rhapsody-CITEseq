...
write.csv(df_prop, file = paste0(path,"/output/13_gated_proportion/CD8T_gate_cluster_proportion_patient.csv"), row.names = F) 
...
df_mat$gate_cluster <- paste0("gate_", df_mat$gate_cluster)
...
ggplot(df_mat, aes(y=cluster_name0.3, x=gate_cluster, fill = prop)) +
  ...
  xlab("") + ylab("")+
  coord_fixed()  
ggsave(file= paste0(path,"/output/13_gated_proportion/heatmap_CD8T_gate_cluster_overlap.png"),
