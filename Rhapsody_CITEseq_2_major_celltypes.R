...
#load the data
pbmc <- readRDS(file = paste0(path, "/output/1_integration/pbmc_integrated.rds"))
...
for (i in cluster.name){
  ...
  ggsave(file = paste0(path,"/output/2_major_clusters/umap_comparison1/DimPlot_pbmc_", i, ".png"),
...
ggsave(file = paste0(path, "/output/2_major_clusters/UMAP_major_clusters_initial.png"), 
       width=10, height=8, units = "in", bg = 'white')

saveRDS(pbmc, file = paste0(path, "/output/2_major_clusters/pbmc_major_celltype_initial.rds"))

# Feature plots
Idents(pbmc) <- "cluster_0.3"
for(i in markers){
  FeaturePlot(pbmc, features =i, cols =  c(alpha("lightgrey",0.4), "blue"), min.cutoff =0 ,max.cutoff =5)
  ggsave(file = paste0(path, "/output/2_major_clusters/FeaturePlot_1/FeaturePlot_", i, ".png"), 
...
for (i in cluster.name){
  DimPlot(pbmc, group.by = i, label = T, repel = T, pt.size = 1, label.size = 7, )
  ggsave(file = paste0(path,"/output/2_major_clusters/umap_comparison2/DimPlot_pbmc_", i, ".png"),
         height = 7, width = 8, units = "in", bg = 'white')
}

DimPlot(pbmc, group.by ="cluster_0.2",label = T, repel=TRUE, label.size = 5) + 
  ggsci::scale_color_d3(palette="category20") +ggtitle("")
ggsave(file = paste0(path, "/output/2_major_clusters/UMAP_major_clusters_cleaned.png"), 
       width=10, height=8, units = "in", bg = 'white')

# *Figure S2B* 
Idents(pbmc) <- "cluster_0.2"
for(i in markers){
  FeaturePlot(pbmc, features =i, cols =  c(alpha("lightgrey",0.2), "blue"),  
                 min.cutoff =0 ,max.cutoff =5) +
    theme(plot.title = element_text(size = 24)) +
    NoAxes()
  ggsave(file = paste0(path, "/output/2_major_clusters/FeaturePlot_2/FeaturePlot_", i, ".png"), 
         width=4.5, height=3.7, units = "in", bg = 'white')
}

# save RDS file 
saveRDS(pbmc, file = paste0(path, "/output/2_major_clusters/pbmc_major_celltype_cleaned.rds"))
...
ggsave(file = paste0(path, "/output/2_major_clusters/UMAP_monaco_SingleR.png"), 
       width=10, height=8, units = "in", bg = 'white')
...
DimPlot(pbmc, group.by ="major_celltype") + ggsci::scale_color_d3(palette="category20") + 
  theme(legend.text = element_text(size = 20),
        axis.title = element_text(size = 20))+
  guides(color = guide_legend(override.aes = list(size=5))) + ggtitle("")
ggsave(file = paste0(path, "/output/2_major_clusters/UMAP_major_celltype.png"), 
       width=10, height=8, units = "in", bg = 'white')
...
ggsave(file = paste0(path, "/output/2_major_clusters/UMAP_major_celltype_Fig1B.png"), 
       width=9, height=7, units = "in", bg = 'white')

## save RDS file 
saveRDS(pbmc, file = paste0(path, "/output/2_major_clusters/pbmc_major_celltype_annotated.rds"))

## DGE *Excel file 3* 
dge <- FindAllMarkers(pbmc, assay = "RNA", slot = "data", 
                      group.by = "major_celltype", only.pos = T, test.use = "MAST", latent.vars = "SampleName") 
write.csv(dge, file = paste0(path, "/output/2_major_clusters/major_celltype_dge_MAST.csv"))
...
## *Figure S2A* 
png(filename = paste0(path, "/output/2_major_clusters/heatmap_major_celltype_ADT.png"), height =5, width =12, res=350, units = "in")
...
write.csv(cell_count, file = paste0(path, "/output/2_major_clusters/cell_count_major_celltype_per_patient.csv"),
          row.names = F)
