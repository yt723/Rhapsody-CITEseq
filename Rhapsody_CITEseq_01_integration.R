...
pbmc.rna <- as.sparse(read.csv("raw_data/Genes_raw.csv", sep=",", header = TRUE, row.names = 1))
...
meta <- read.csv("raw_data/Metadata.csv", row.names = 1)
...
ggsave(file = "output/1_integration/UMAP_Library_Integrated_RNA.png", width=10, height=8, units = "in", bg = 'white')

# save RDS file 
saveRDS(immune.combined, file = "output/1_integration/immune.combined_rna.rds")

## ADT integration
pbmc.adt <- as.sparse(read.csv("raw_data/Ab_raw.csv", sep = ",", header = TRUE, row.names = 1))
...
# save RDS file 
saveRDS(immune.combined_adt, file = "output/1_integration/immune.combined_adt.rds")

# read previously created objects 
immune.combined <- readRDS(file = "output/1_integration/immune.combined_rna.rds")
immune.combined_adt <- readRDS(file = "output/1_integration/immune.combined_adt.rds")
...
saveRDS(immune.combined, file = "output/1_integration/pbmc_integrated.rds")
